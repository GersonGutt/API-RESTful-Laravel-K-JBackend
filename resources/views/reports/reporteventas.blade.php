...
<table class="table">
    ...
    <tbody>
      <tr>
        ...
        <div class="texto-destacado">Reporte de Ventas</div>
        <small class="texto-destacado">Fecha del reporte: </small>
        <h2 class="texto-red">{{ $fechaReporte }}</h2>
        <small class="texto-destacado">Reporte hecho de las fechas: </small>
        <small class="texto-destacado">{{ $fechaInicio }} - {{ $fechaFinal }}</small>
      </td>
      </tr>
    </tbody>
  </table>
  <table class="table table-striped bordes-table">
    ...
    <tr>
    <th class="center texto-destacado col-4">CLIENTE</th>
    <th class="texto-destacado col-2">DUI</th>
    <th class="texto-destacado">NUMERO DE FACTURA </th>
    <th class="right texto-destacado">FECHA DE LA VENTA</th>
    <th class="center texto-destacado">TOTAL</th>
    </tr>
    <tbody>
      @foreach ($ventas as $item)
      <tr>
        <td class="center texto-destacado">{{ $item['cliente'] }}</td>
        <td class="left strong texto-destacado">{{ $item['dui'] }}</td>
        <td class="left texto-destacado">000{{  $item['numerofactura'] }}</td>
        <td class="right texto-destacado"> {{ $item['fechaVenta'] }} </td>
        <td class="center texto-destacado"> ${{ $item['total'] }} </td>
        </tr>
      @endforeach
      @if ($ventas == []){
...
<div class="card-footer bg-white mt-0">
   <p class="texto-destacado"><strong>Total en ventas: ${{ $total }}</strong></p>
    </div>
